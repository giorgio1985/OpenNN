...
TensorTest::TensorTest() : UnitTesting()
{   
}
...
void TensorTest::test_constructor()
{
   ...
   assert_true(t1.size() == 0, LOG);

   // Size

   Tensor<bool> t2(1);

   assert_true(t2.size() == 1, LOG);

   // Size initialization

   Tensor<bool> t3(Vector<size_t>{1}, false);

   assert_true(t3.size() == 1, LOG);
   assert_true(t3[0] == false, LOG);

   // File

   // Copy

   Matrix<bool> matrix(3,3,true);

   Tensor<bool> t4(matrix);

   assert_true(t4.get_dimension(0) == 3, LOG);
   assert_true(t4.get_dimension(1) == 3, LOG);
   assert_true(t4.get_dimensions() == Vector<size_t>{3,3}, LOG);

}


void TensorTest::test_destructor()
{
}


void TensorTest::test_sum_operator()
{
   ...
   a.set(Vector<size_t>{1}, 1);

   b = a + 1;

   c.set(Vector<size_t>{1}, 2);
   ...
   a.set(Vector<size_t>{1}, 1);
   b.set(Vector<size_t>{1}, 1);

   c = a + b;

   d.set(Vector<size_t>{1}, 2);

   assert_true(c == d, LOG);
}


void TensorTest::test_rest_operator()
{
   ...
   a.set(Vector<size_t>{1}, 1.0);
   b = a - 1.0;

   c.set(Vector<size_t>{1}, 0.0);
   ...
   a.set(Vector<size_t>{1}, 1.0);
   b.set(Vector<size_t>{1}, 1.0);

   c = a - b;

   d.set(Vector<size_t>{1}, 0.0);

   assert_true(c == d, LOG);
}


void TensorTest::test_multiplication_operator()
{
   ...
   t.set(Vector<size_t>{3,2}, 1.0);
   a.set(Vector<size_t>{1,3}, 1.0);
}/*
   p = a*t;

   assert_true(p.get_dimension(0) == 3, LOG);
   assert_true(p.get_dimension(1) == 2, LOG);
   assert_true(p == 1.0, LOG);
}


void TensorTest::test_division_operator()
{
   ...
   c.set(Vector<size_t>{1}, 0.5);
   
   assert_true(b == c, LOG);

   // Vector

   a.set(Vector<size_t>{1}, 2.0);
   b.set(Vector<size_t>{1}, 2.0);
